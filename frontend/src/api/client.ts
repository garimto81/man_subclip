...
class ApiClient {
  ...
  /**
   * Create clip directly from GCS (streaming, no full download)
   * Uses HTTP Range Request - 99% faster than full download
   */
  async createClipFromGCS(request: GCSClipCreateRequest): Promise<GCSClipResponse> {
    const response = await this.client.post<GCSClipResponse>(
      '/clips/create-from-gcs',
      null,
      {
        params: {
          ...
          padding_sec: request.padding_sec ?? 0,
        },
        timeout: 300000, // 5 minutes for GCS streaming extraction
      }
    )
    return response.data
  }
  ...
  /**
   * Get direct download URL for a clip
   */
  getClipDirectDownloadUrl(clipId: string): string {
